...
from config1 import verify_code_params, verify_code_headers, lecture_headers, lecture_key
import warnings
...
def keep_alive(wid):
    ...
    res = r.json()
    if res['code'] != 0:
        print('保活失效，请检查cookie！', "当前时间: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])

    print('用户身份有效，登录状态保活', "当前时间: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
...
def rob(wid):
    ...
    """
    time.sleep(0.3)
    print("定时预约任务开始, wid: ", wid, datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])

    # 获取验证码图片
    verify_code_img_base64 = get_lecture_verify_code(wid)
    print("获取验证码，当前时间: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])

    # 解析验证码
    verify_code = parse_verify_code(verify_code_img_base64)
    print("解析验证码成功: ", verify_code, "当前时间: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
    # 尝试预约讲座
    res = reserve_lecture(wid, verify_code)
    print("预约结果: ", res, "当前时间: ", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3])
...
if __name__ == "__main__":
    ...
    scheduler.add_job(rob, 'cron', hour=19, minute=0, second=0, args=[lecture['WID']])
    scheduler.start()
